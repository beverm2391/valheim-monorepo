...
message_patch="$(sed -n '/\[HarmonyPatch(typeof(Player), nameof(Player.Message)/,/internal static class BuildingRepairMessagePatch/p' "$patches")"
grep -Fq 'typeof(Sprite),' <<<"$message_patch"
grep -Fq 'typeof(bool),' <<<"$message_patch"
