...
grep -Fq '"Auto-pickup reach"' "$catalog_file"
grep -Fq "Collect ordinary dropped items from 2× Valheim's native range when auto-pickup is on, including items floating in water or tar. Inventory and weight limits stay native" "$catalog_file"
grep -Fq 'Manually pick up ordinary items submerged in native tar pits, or collect them with auto-pickup' "$catalog_file"
...
grep -Fq 'Positive gains are doubled; perfect defenses still show while adrenaline is locked and add the actual gain when available' "$catalog_file"
...
grep -Fq 'it closes after your choice and highlights the size when reopened' "$catalog_file"
grep -Fq '"Left Shift + Hoe"' "$catalog_file"
grep -Fq 'Preview and apply each Hoe terrain action at 3× its native radius; releasing Shift immediately restores the native radius' "$catalog_file"
grep -Fq 'FarmingSettings.DefaultGridSize' "$catalog_file"
grep -Fq 'each game session; your choice persists when the picker reopens' "$catalog_file"
grep -Fq 'FarmingSettings.BerryGridSpacing' "$catalog_file"
grep -Fq 'spacing in both preview and placement at every grid size' "$catalog_file"
