...
grep -Fq '"Combat"' "$catalog_file"
grep -Fq 'new Entry("bh debug comfort", "Record one native comfort calculation without changing the player, furniture, or world")' "$catalog_file"
grep -Fq "The comfort command records every furniture candidate exposed by Valheim's native query" "$catalog_file"
