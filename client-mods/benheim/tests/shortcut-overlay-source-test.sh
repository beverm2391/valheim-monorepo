...
grep -Fq "the 9x9 uses each native bush's collider footprint for spacing" "$catalog_file"
...
grep -Fq 'AffinityApplication.TestResourceAmount' "$catalog_file"
grep -Fq 'the Affinity stays with that item' "$catalog_file"
...
grep -Fq 'destroys the old Affinity and its prior investment without a refund' "$catalog_file"
grep -Fq '"Combat"' "$catalog_file"
grep -Fq 'new Entry("bh debug comfort", "Show what contributed, what Valheim ignored, and what is just outside comfort range")' "$catalog_file"
grep -Fq 'The comfort command prints a readable summary and records the same complete evidence in diagnostics' "$catalog_file"
grep -Fq "Pieces hidden before Valheim's native query cannot appear" "$catalog_file"
