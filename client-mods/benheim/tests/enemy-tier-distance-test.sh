...
rg -Uq '\[HarmonyPatch\][\s\S]*internal static class WildernessMapHover' "$map_hover"
rg -Fq 'BitArray ___m_explored' "$map_hover"
rg -Fq 'BitArray ___m_exploredOthers' "$map_hover"
rg -Fq 'ZInput.pointerPosition' "$map_hover"
rg -Fq 'typeof(Vector3),' "$patches"
rg -Uq '"Spawn",\n        typeof\(SpawnSystem\.SpawnData\),\n        typeof\(Vector3\),\n        typeof\(bool\)\)\]' "$patches"
if rg -n 'levelUpMultiplier|NativeLevelUpMultiplier' "$patches" "$chance"; then
  printf '%s\n' "Enemy tier chance must use 1.0 Spawn's direct level-up chance path" >&2
  exit 1
fi
...
rg -Uq '\[HarmonyPatch\][\s\S]*internal static class WildernessDangerPresentationPatches' "$danger_presentation_patches"
...
rg -Uq '\[HarmonyPatch\][\s\S]*internal static class BoarTierIdentityPatches' "$boar_patches"
...
rg -Fq 'UpdateSpawnList(spawnList.m_spawners, time, eventSpawners: false, "b_")' "$native_spawn"
rg -Fq 'UpdateSpawnList(currentSpawners, time, eventSpawners: true, "e_")' "$native_spawn"
