...
rg -Fq 'if (AffinityState.Read(weapon) != AffinityLoadResult.Lunge) return;' "$feature/LungeRuntime.cs"
...
rg -Fq '? "Lunge Applied"' "$feature/AffinityForgeUi.cs"
rg -Fq '? "This exact Club already has Lunge."' "$feature/AffinityForgeUi.cs"
rg -Fq 'dash 10 m/s forward and raise you to at least +3 m/s vertically' "$feature/AffinityForgeUi.cs"
confirm_apply="$(sed -n '/internal void ConfirmApply()/,/private void CompleteApply/p' "$feature/AffinityForgeUi.cs")"
[[ "$confirm_apply" == *'if (AffinityRules.IsSameAffinity(AffinityState.Read(captured), AffinityLoadResult.Lunge))'*'return;'*'UnifiedPopup.Push(new YesNoPopup('* ]]
...
rg -Fq 'Lunge applied. The 1 Wood cost is nonrefundable.' "$feature/AffinityForgeUi.cs"
