...
- The board shows the portal tag on both sides with the existing glowing
  ...
  tag and updates when the tag changes. The complete tag stays readable inside
  the board on both sides, with space around the letters. A portal with an empty
  tag shows no board.
...
Ben accepted the wooden sign style, placement, and text fitting in live play.
