...
## First playable affinity: Snipe

Snipe is the first Bow affinity. It can prove the full path from acquisition
through permanent combat use on any base-game bow. It is an affinity applied to
a bow, not a separate custom bow.

A Snipe bow always favors deliberate long-range shots. When the player draws
the bow, it always uses Snipe's presentation and handling. It does not offer a
second normal mode. Long-range precision and skilled headshots should become
exceptional. Slower handling and restricted awareness should make the same bow
worse when enemies close the distance.

The exact unlock, recipe, scope presentation, range benefit, handling cost, and
headshot behavior remain open. Do not extend this design to additional
affinities until Snipe proves that permanent specialization and resource
investment improve real gameplay.

## Status

Affinities are in product design. No affinity application, persistence, or
combat behavior is implemented yet.
