...
The MCP launcher locates its source from its own path. `VALHEIM_DEV_ROOT`
remains the explicit runtime-data override. A missing data root reports the
resolved path in the error. Valheim Dev adds no broader path-discovery system.
