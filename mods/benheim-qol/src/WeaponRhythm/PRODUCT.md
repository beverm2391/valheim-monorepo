...
- Airborne Melee is an experimental approach technique: make space, carry
...
- A qualified hit deals `1.15x` all native damage and uses `2x` its native
  stagger multiplier. Both values are playtest tuning, not accepted balance.
- A qualified outcome shows `PERFECT IMPACT` in Benheim's local feedback lane.
  ...
  one shake, not one per target. The semantic confirmation remains visible
...
- Diagnostics record each qualified local hit and each `grounded`,
  ...
  speed, toward-target speed, both thresholds, both multipliers, and whether
  same-outcome presentation was coalesced, but do not log per frame.
