...
rg -Fq 'AirborneMelee.BeginAttackAttempt(' "$patches"
rg -Fq 'AirborneMelee.CompleteAttackAttempt(__state, ___m_currentAttack, __result)' "$patches"
rg -Fq '[HarmonyPatch(typeof(Attack), nameof(Attack.Update))]' "$patches"
rg -Fq 'AirborneMelee.ObserveAttackProgress(__instance)' "$patches"
...
rg -Fq 'internal static AirborneMeleeStartAttempt? BeginAttackAttempt(' "$logic"
rg -Fq 'internal static void CompleteAttackAttempt(' "$logic"
...
rg -Fq '"native_start_rejected"' "$logic"
rg -Fq '"grounded_at_start"' "$logic"
rg -Fq '.String("weapon", attempt.Weapon)' "$logic"
rg -Fq '.String("attack_control", attempt.AttackControl)' "$logic"
rg -Fq '.String("attack_animation", attempt.AttackAnimation)' "$logic"
rg -Fq '.Boolean("native_attack_started", nativeAttackStarted)' "$logic"
rg -Fq '.Boolean("start_grounded", attempt.StartedGrounded)' "$logic"
rg -Fq 'if (!attempt.StartedGrounded && attempt.FreshInput)' "$logic"
rg -Fq 'new AirborneMeleeSwingState(attempt, armed: false, startGateObserved: false)' "$logic"
...
rg -Fq '.String("operation_phase", eventName == "airborne_melee_armed" ? "start" : "terminal")' "$logic"
...
rg -Fq 'if ((InAttack() && !HaveQueuedChain()) || InDodge() || !CanMove() || IsKnockedBack() || IsStaggering() || InMinorAction())' "$native_humanoid"
...
rg -Fq 'if ((m_queuedAttackTimer > 0f || m_attackHold) && StartAttack(null, secondaryAttack: false))' "$native_player"
rg -Fq 'if ((m_queuedSecondAttackTimer > 0f || m_secondaryAttackHold) && StartAttack(null, secondaryAttack: true))' "$native_player"
...
rg -Fq 'DoMeleeAttack();' "$native_attack"
rg -Fq 'case AttackType.Area:' "$native_attack"
rg -Fq 'DoAreaAttack();' "$native_attack"
