...
quick_stack_top_left_layout="$root/src/Inventory/QuickStackTopLeftLayout.cs"
...
grep -Fq 'QuickStackTopLeftLayout.MoveBelowHotbar()' "$quick_stack_feedback"
grep -Fq 'FallbackTopOffset = 120f' "$quick_stack_top_left_layout"
grep -Fq 'hotbarRect.GetWorldCorners' "$quick_stack_top_left_layout"
grep -Fq 'Screen.height - bottomLeft.y' "$quick_stack_top_left_layout"
grep -Fq 'rect.pivot = new Vector2(0f, 1f)' "$quick_stack_top_left_layout"
grep -Fq 'TextAlignmentOptions.TopLeft' "$quick_stack_top_left_layout"
