...
grep -Fq "Join-Path \$updaterDir 'launch-windows.ps1'" "$installer"
...
grep -Fq "steam://rungameid/892970" "$launcher"
grep -Fq "Update and launch" "$launcher"
grep -Fq "Launch current version" "$launcher"
grep -Fq "releases/latest/download/VERSION" "$launcher"
grep -Fq "Start-Process 'steam://rungameid/892970'" "$launcher"
...
grep -Fqx "Benheim-Windows-$version/launch-windows.ps1" <<<"$package_entries"
