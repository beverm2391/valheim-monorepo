...
grep -Fq '"$root/scripts/verify.sh"' "$release_script"
grep -Fq 'BENHEIM_QOL_SKIP_BUILD=1 "$root/scripts/package-macos.sh"' "$release_script"
grep -Fq 'BENHEIM_QOL_SKIP_BUILD=1 "$root/scripts/package-windows.sh"' "$release_script"
