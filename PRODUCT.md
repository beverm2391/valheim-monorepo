...
| Benheim Server Support | Coordinates Put Away and keeps each player's confirmed-kill chain on the server. | Server | Benheim `0.1.64` requires Benheim Server Support `0.1.2` for Put Away and BERSERKER/SLAUGHTERHOUSE. Kill Attribution V2 does not interoperate with older client or server components. |
