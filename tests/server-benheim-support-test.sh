...
kill_delivery_queue="$mod/src/KillChainDeliveryQueue.cs"
kill_delivery_attempt="$mod/src/KillChainDeliveryAttempt.cs"
kill_delivery_runtime="$mod/src/KillChainDeliveryRuntime.cs"
...
grep -Fq 'WindowSeconds = 10d' "$kill_chain"
grep -Fq 'BerserkerThreshold = 3' "$kill_chain"
grep -Fq 'SlaughterhouseThreshold = 6' "$kill_chain"
grep -Fq 'KillChainDeliveryRuntime.Update(serverTimeSeconds, Chains)' "$kill_server"
grep -Fq 'maximumPerKiller' "$kill_delivery_queue"
grep -Fq 'killerPeer.m_rpc.IsConnected()' "$kill_delivery_runtime"
