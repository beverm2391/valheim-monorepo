...
mod_root="$root/server-mods/benheim-eternal-fire"
...
dotnet build "$mod_root/src/BenheimEternalFire.csproj" \
  --configuration Release \
  -p:ValheimManagedDir="$managed_dir" \
  -p:BepInExCoreDir="$bepinex_core_dir"

install -m 0644 \
  "$mod_root/src/bin/Release/netstandard2.1/BenheimEternalFire.dll" \
  "$mod_root/dist/BenheimEternalFire.dll"

shasum -a 256 "$mod_root/dist/BenheimEternalFire.dll"
