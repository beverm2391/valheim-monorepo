...
administrator using a compatible Benheim client. `bh spawn boar 0` creates one
native unstarred Boar as a control. `bh spawn boar 1` creates one native
one-star Boar. `bh spawn boar 2` creates one native two-star Boar. `bh help` lists
...
Using the commands requires the matching Benheim client command on the
requesting player and Benheim Test Commands `0.1.4` on the dedicated server.
Valheim 1.0 QA confirmed that each of the three spawn requests was accepted
once and created native Boar levels `1`, `2`, and `3` for requested stars `0`,
`1`, and `2`.
