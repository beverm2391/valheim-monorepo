...
{
    internal const string EventFileName = "BenheimServerSupportEvents.ndjson";
    ...
    private static void LogFailure(Exception exception)
    {
        ...
        failureLogged = true;
        Plugin.Log.LogWarning($"Benheim server-support diagnostics are unavailable: {exception.GetType().Name}");
    }
}
